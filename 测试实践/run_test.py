    ...
    def run_test(self):
        ...
        for i in range(row_counts):
            ...
            if self.util.is_equal_code(state,response):
                ...
                test_fail+=1
            self.data.write_all_data(lines,lines,name,url,method,str(headers[i]),all_test[i],note[i],state,response,test_res)
            lines+=1
