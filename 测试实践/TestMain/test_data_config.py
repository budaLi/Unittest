...
class TestDataConfig:
    ...
    def __init__(self,file_name=None):
        if file_name is None:
            file_name= "C:/Users/lenovo/PycharmProjects/自动化测试/测试实践/Excel/test_excel/test_UserAddUser.xls"
    ...
    def get_note_col(self):
        ...
        num =self.opExcel.get_cel_value(3,1)
    ...
    def refactor_all_test(self):
        ...
        if res:
            ...
            for i in range(self.test_num):
                tem = []
                dic={}
                for j in range(self.test_zidaun_init_position[1],self.test_zidaun_init_position[1]+self.test_num+1):
                    tem.append(self.opExcel.get_cel_value(row,j))
                    dic = {key:word for key in ziduan_name for word in tem}
                res.append(dic)
            return res


if __name__=="__main__":
    testDataConfig = TestDataConfig("./Excel/test_UserAddUser.xls")
